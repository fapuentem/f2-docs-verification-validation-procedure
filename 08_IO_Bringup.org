...
#+BEGIN_SRC bash :results output :exports both
  echo "--- ALIASES ---"
  for f in /proc/device-tree/aliases/i2c* /proc/device-tree/aliases/serial* /proc/device-tree/aliases/spi* 2>/dev/null; do
    [ -e "$f" ] && printf "%-9s -> %s\n" "$(basename "$f")" "$(tr -d '\0' <"$f")"
  done
  ...
  echo "--- SERIAL ---"
  sudo dmesg -T | egrep -i "tty|serial" | tail -n 10
  ls -l /dev/ttyTHS* /dev/ttyTCU* /dev/ttyAMA* 2>/dev/null
...
#+BEGIN_SRC bash :dir / :sudo yes :results output :exports both
  ...
  if [ "$COUNT" -gt 0 ]; then
    ...
    echo "        sudo ip link set $IFACE up type can bitrate 500000"
